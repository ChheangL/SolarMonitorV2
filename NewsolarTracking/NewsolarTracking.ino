...
void setup() {
  ...
  servoY.write(30);      //Rotate CCW (servo Motor in front)
  delay(5000);
...
#ifdef SERIALPRINT
  if(i == LDR1) value_ldr[0] = value_ldr[0] * 1.04;
  if(i == LDR2) value_ldr[1] = value_ldr[1] * 1.06;
     Serial.print(((String)i)+":");
    Serial.print(value_ldr[i-LDR1]);
    Serial.print(" ");
#endif
}
avg[0] = ((value_ldr[0]+value_ldr[1])/2); //right averge
avg[1] = ((value_ldr[3]+value_ldr[2])/2); //left averge
avg[2] = ((value_ldr[0]+value_ldr[3])/2); //top averge
avg[3] = ((value_ldr[1]+value_ldr[2])/2); //bottom averge
